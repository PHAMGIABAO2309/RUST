...
async fn main() {
    ...
    let hello_route = route::create_hello_route(pool.clone());

    let routes = hello_route.with(warp::log("hello_rust2"));
    ...
    let server = warp::serve(routes.or(register_route).or(static_files).or(call_login)) // 👉 Chạy server
        .run(([127, 0, 0, 1], 8080));
